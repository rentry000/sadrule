...
# 处理单个文件的函数
def process_file(file_url, result_queue):
    try:
        # 下载文件
        response = requests.get(file_url)
        response.raise_for_status()  # 检查下载是否成功
        lines = response.text.splitlines()  # 分割成行
        ...
        for line in lines:
            ...
            if '/' in line:
                ...
                # 判断是 IPv4 还是 IPv6 并添加后缀
                try:
                    ip = ipaddress.ip_address(line)
                    if ip.version == 4:
                        processed_lines.append(f"{line}/32")
                    elif ip.version == 6:
                        processed_lines.append(f"{line}/128")
                except ValueError:
                    continue  # 跳过无效行
...
def main(file_urls):
    ...
    for url in file_urls:
        t = threading.Thread(target=process_file, args=(url, result_queue))
        t.start()
        threads.append(t)
